...
Every enabled MCP server injects its tool schema into **every turn**, regardless of whether you call any of its tools. Heavyweight servers can cost 20k+ tokens per turn each — often dwarfing whatever GSD itself can save through `model_profile` tuning. This is a Cline harness concern, not a GSD concern: GSD does **not** manage MCP enablement. The toggle lives in `.cline/settings.json` under `enabledMcpjsonServers` and `disabledMcpjsonServers`.
...
The keys live in `.cline/settings.json` (project) or `C:/Users/wikiepeidia/OneDrive - caugiay.edu.vn/bài tập/usth/GEN14/GROUP project/Group-project-AI-ML/settings.json` (global) — **not** in `.planning/config.json`:
