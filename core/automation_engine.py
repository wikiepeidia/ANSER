    ...
    def _run_scheduler(self):
        while self.running:
            try:
                ...
                logger.error("Error in automation scheduler: %s", e, exc_info=True)
    ...
    def check_scheduled_automations(self):
        ...
        try:
            ...
            for row in rows:
                ...
                if should_run:
                    logger.info("Running scheduled automation %s", auto_id)
            ...
            logger.error("Error checking scheduled automations: %s", e, exc_info=True)
        finally:
            conn.close()
    ...
    def check_low_stock(self, product_id, current_stock):
        ...
        try:
            ...
            for row in rows:
                ...
                if should_trigger:
                    logger.info("Triggering low stock automation %s for product %s", auto_id, product_id)
            ...
            logger.error("Error checking low stock for product %s: %s", product_id, e, exc_info=True)
        finally:
            conn.close()
    ...
    def execute_import_automation(self, auto_id, config, product_id):
        ...
        try:
            ...
            c.execute('''INSERT INTO import_transactions
                            ...
                            VALUES (?, ?, ?, ?, ?, ?)''',
                        (code, supplier_name, total_price, 'pending', f'Auto-generated by automation #{auto_id}', 1))
            ...
            conn.commit()
            logger.info("Created automated import %s for product %s", code, product_id)

        except Exception as e:
            logger.error("Error executing import automation %s for product %s: %s", auto_id, product_id, e, exc_info=True)
            conn.rollback()
        finally:
            conn.close()
    ...
    def execute_scheduled_import(self, auto_id, config):
        ...
        try:
            ...
            for prod in low_stock_products:
                p_id = prod['id']
                price = prod['price'] if prod['price'] else 0
            ...
            c.execute('''INSERT INTO import_transactions
                            (code, supplier_name, total_amount, status, notes, created_by)
                            VALUES (?, ?, ?, ?, ?, ?)''',
                        (code, supplier_name, total_amount, 'pending', f'Scheduled Import #{auto_id}', 1))
            import_id = c.lastrowid
            ...
            conn.commit()
            logger.info("Created scheduled import %s with %d items", code, len(items))

        except Exception as e:
            logger.error("Error executing scheduled automation %s: %s", auto_id, e, exc_info=True)
            conn.rollback()
        finally:
            conn.close()
