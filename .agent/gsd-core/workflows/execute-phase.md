...
@.agent/gsd-core/references/context-budget.md
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @.agent/gsd-core/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @.agent/gsd-core/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `.agent/gsd-core/workflows/execute-plan.md` **inline**
...
per-agent branch namespace `worktree-agent-*`, NOT the `.agent/worktrees/` path: the
orchestrator may itself be legitimately invoked from a feature worktree under
`.agent/worktrees/`, so a path-substring refusal would break legitimate runs. Do NOT
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       @.agent/gsd-core/references/checkpoints.md
       @.agent/gsd-core/references/tdd.md
       @.agent/gsd-core/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@.agent/gsd-core/references/executor-examples.md'}
       </execution_context>
       ...
       - ${PROJECT_ROOT}/.agent/skills/ or ${PROJECT_ROOT}/.agents/skills/ (Project skills, if either exists — list skills, read SKILL.md for each, follow relevant rules during implementation)
       </files_to_read>
...
Read and follow `.agent/gsd-core/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
