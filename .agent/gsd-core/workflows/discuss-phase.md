...
<required_reading>
@.agent/gsd-core/references/domain-probes.md
@.agent/gsd-core/references/gate-prompts.md
@.agent/gsd-core/references/universal-anti-patterns.md
</required_reading>
...
if [ -f ".agent/gsd-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
```bash
SPIKE_FINDINGS=$(ls ./.agent/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
SKETCH_FINDINGS=$(ls ./.agent/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1 || true)
...
Read `@.agent/gsd-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
