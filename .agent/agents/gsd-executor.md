...
@.agent/gsd-core/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@.agent/gsd-core/references/ios-scaffold.md
...
@.agent/gsd-core/references/executor-examples.md
</deviation_rules>
...
**See @.agent/gsd-core/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `@.agent/gsd-core/references/execute-mvp-tdd.md`. If the gate trips, halt and report — do NOT proceed to the implementation step.
...
**Use template:** @.agent/gsd-core/templates/summary.md
