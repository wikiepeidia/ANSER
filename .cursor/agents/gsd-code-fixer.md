...
**Project instructions:** Read `.cursor/rules/` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during fixes.
...
Read `.cursor/rules/` and check for `.cursor/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO respect .cursor/rules/ project conventions** during fixes. If project requires specific patterns (e.g., no `any` types, specific error handling), apply them.
...
- [ ] Project conventions from .cursor/rules/ respected during fixes
