...
from core.security import env_flag, is_local_environment  # noqa: E402
...
def create_app():
    ...
    local_runtime = is_local_environment(Config)
    secure_runtime = not local_runtime
    app.config['SESSION_COOKIE_SECURE'] = env_flag('SESSION_COOKIE_SECURE', secure_runtime)
    app.config['SESSION_COOKIE_HTTPONLY'] = True
    app.config['SESSION_COOKIE_SAMESITE'] = 'Lax'
    # Share the cookie with Retail/Sản xuất on their own subdomains — without
    # this, Flask scopes the cookie to the exact host that issued it, and
    # the whole point of this app (single login, both mảng trust the
    # cookie) breaks the moment they're on different subdomains instead of
    # sharing one host like 127.0.0.1 in local dev.
    app.config['SESSION_COOKIE_DOMAIN'] = os.environ.get(
        'SESSION_COOKIE_DOMAIN',
        '127.0.0.1' if local_runtime else None,
    )
    app.config['PREFERRED_URL_SCHEME'] = 'https' if env_flag('FORCE_HTTPS', secure_runtime) else 'http'
    app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(days=7)
    app.config['SESSION_REFRESH_EACH_REQUEST'] = True
    app.config['RATELIMIT_ENABLED'] = env_flag('RATELIMIT_ENABLED', secure_runtime)
