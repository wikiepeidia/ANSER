...
- **Masking convention: `****<last-4>`** (e.g. `sk-abc123def456` → `****f456`).
  ...
  as `(unset)`.
- **Plaintext is never echoed by conversational prompting descriptions, confirmation
...
`TEXT_MODE=true` and replace every `conversational prompting` call with a plain-text
