...
Use conversational prompting with current values pre-selected. Questions are grouped into six visual sections; the first question in each section carries the section-denoting `header` field (conversational prompting renders abbreviated section tags for grouping, max 12 chars).
...
```
// Model profile is selected via a two-question split because conversational prompting enforces a
...
conversational prompting([
  ...
  // the conversational prompting call and do not touch the existing workflow.code_review_depth value.
  {
